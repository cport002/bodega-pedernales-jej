...
export default function DevolucionesPage() {
  ...
  useEffect(() => {
    api.get('/devoluciones').then(r => { setDevoluciones(r.data); setLoading(false) }).catch(() => setLoading(false))
  }, [])
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="Devoluciones" subtitle={`${devoluciones.length} devolución(es) registradas`} icon={Undo2} />
            ...
            <tbody>
              {devoluciones.map(d => (
                <tr key={d.id} className="table-row">
                  ...
                  <td className="table-cell">{d.motivo || '-'}</td>
              ...
              {devoluciones.length === 0 && (
                <tr><td colSpan={7} className="table-cell text-center text-gray-400 py-8">Sin devoluciones registradas todavía</td></tr>
              )}
            </tbody>
          </table>
        </div>
